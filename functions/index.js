...
//pay money by URL
app.get('/user/:id/pay/:amount', (req, res) => {
	res.set("Content-Type", "application/json;charset=UTF-8");
	...
	user.once('value', snapshot => {
		...
		if (snapshot.val() !== null) {
			...
			if (currentAmount >= parseFloat(req.params.amount)) {
				currentAmount -= parseFloat(req.params.amount);
				user.update({ balance: currentAmount }, () => {
					res.status(200).send(`{"Message":"OK!"}`);
				});
			} else {
				res.status(400).send(`{"error":"user ${req.params.id} does not have enough balance", "Balance": "${currentAmount}"}`)
			}
		}
		else {
			res.status(404).send(`{"error":"user ${req.params.id} does not exit"}`)
		}
	});
});
...
//add money by URL
app.get('/user/:id/add/:amount', (req, res) => {
	res.set("Content-Type", "application/json;charset=UTF-8");
